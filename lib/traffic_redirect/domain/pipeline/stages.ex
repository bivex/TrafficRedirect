...
defmodule TrafficRedirect.Domain.Pipeline.Stages do
  @moduledoc """
  All 30 Click Pipeline stages matching the Technical Specification.
  """
  ...
  defmodule StaticServingStage do
    ...
    def execute(%Payload{request: req} = payload, _ctx) do
      ...
      if is_list(path) and List.last(path) && String.match?(List.last(path), ~r/\.(css|js|png|jpg|jpeg|gif|svg|ico|woff|woff2|ttf)$/i) do
        # Static asset request - offload with X-Accel-Redirect header
  ...
  # 2. CheckCacheStage
  defmodule CheckCacheStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    def execute(%Payload{} = payload, _ctx) do
      # Verifies entity cache status; warms up if needed
      payload
    end
  end

  # 3. HttpsRedirectStage
  defmodule HttpsRedirectStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    def execute(%Payload{request: req} = payload, _ctx) do
  ...
  defmodule CheckPrefetchStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    def execute(%Payload{request: req} = payload, _ctx) do
  ...
  # 5. FindCampaignStage
  defmodule FindCampaignStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
  ...
  defmodule NoCampaignCatcherStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
  ...
  defmodule CheckBypassCacheStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    def execute(%Payload{request: req} = payload, _ctx) do
  ...
  defmodule FillClickInformationStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    def execute(%Payload{request: req, raw_click: click} = payload, ctx) do
      ...
      # Sub IDs extraction (sub_id_1..sub_id_15, subid1..subid15)
      ...
      # Detectors (Geo, Device, Bot, Proxy) via Port Adapters in ctx
  ...
  defmodule ParamsPreprocessStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    def execute(%Payload{} = payload, _ctx), do: payload
  end

  # 10. GenerateVisitorCodeStage
  defmodule GenerateVisitorCodeStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
  ...
  defmodule GenerateSubIdStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    def execute(%Payload{raw_click: click} = payload, _ctx) do
  ...
  defmodule LoadOrCreateSessionStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
  ...
  defmodule CheckParamAliasesStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    def execute(%Payload{} = payload, _ctx), do: payload
  end

  # 14. UpdateCampaignUniquenessStage
  defmodule UpdateCampaignUniquenessStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
  ...
  defmodule ChooseStreamStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    def execute(%Payload{stream: nil, campaign: campaign, raw_click: click} = payload, ctx) do
      ...
      # Order: FORCED (Position) -> REGULAR (Bound / Position / Weight) -> DEFAULT
      chosen_stream =
  ...
  defmodule ApplyStreamActionStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
  ...
  defmodule UpdateStreamUniquenessStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    def execute(%Payload{raw_click: click} = payload, _ctx) do
      # Stream uniqueness calculation
      %{payload | raw_click: %{click | is_unique_stream: click.is_unique_campaign}}
    end
  end

  # 18. ChooseLandingStage
  defmodule ChooseLandingStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    def execute(%Payload{landing: nil, stream: stream} = payload, ctx) when not is_nil(stream) do
      ...
      landings =
        cond do
          stream.landings && stream.landings != [] ->
            stream.landings

          landing_repo ->
            landing_repo.get_by_stream_id(stream.id)
  ...
  defmodule ChooseOfferStage do
    ...
    def execute(%Payload{offer: nil, stream: stream} = payload, ctx) when not is_nil(stream) do
      ...
      offers =
        cond do
          stream.offers && stream.offers != [] ->
            stream.offers

          offer_repo ->
            offer_repo.get_by_stream_id(stream.id)
      ...
      if chosen_offer do
        ...
        # Offer action overrides stream action
        %{
          payload
          | offer: chosen_offer,
            raw_click: raw_click,
  ...
  defmodule GenerateTokenStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    def execute(%Payload{raw_click: click} = payload, _ctx) do
  ...
  defmodule FindAffiliateNetworkStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
  ...
  defmodule UpdateHitLimitStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    def execute(%Payload{} = payload, _ctx), do: payload
  end

  # 23. UpdateCostsStage
  defmodule UpdateCostsStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
  ...
  defmodule UpdatePayoutStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
  ...
  defmodule PrepareRawClickToStoreStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    def execute(%Payload{raw_click: click} = payload, _ctx) do
  ...
  defmodule CheckSendingToAnotherCampaignStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    ...
    def execute(%Payload{action_type: action_type, chain_count: count} = payload, ctx)
      ...
      if count >= @max_chain_depth do
        ...
        if target_campaign do
          # Clean payload and restart pipeline for target campaign
  ...
  # 28. UpdateTokenStage
  defmodule UpdateTokenStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    def execute(%Payload{} = payload, _ctx), do: payload
  end

  # 29. ExecuteActionStage
  defmodule ExecuteActionStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
    def execute(%Payload{response: nil, is_api_request: true} = payload, _ctx) do
      # Click API returns JSON response with metadata
  ...
  defmodule SaveRawClicksStage do
    @behaviour TrafficRedirect.Domain.Pipeline.Stage
